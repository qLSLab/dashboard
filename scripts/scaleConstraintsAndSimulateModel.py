...
# lScalingFactors = [1] + gL.getPrimeNumberInRange(2, 20)
lScalingFactors = gL.getPrimeNumberInRange(20, 50)

gL.toLog(logStrm, f"\n-------------------------------------\nSCALED ONLY MEDIUM COMPONENTS\n-------------------------------------\n")

for scalingFactor in lScalingFactors:
    gL.toLog(logStrm, f"\n-------------------------------------\nScaling factor of wet data values: {scalingFactor}")
    ...
    for timePointRow in dfConsProd.itertuples():
        ...
        constrainedModel = cb.io.read_sbml_model(os.path.join(MODELDIR, modelName_wExpDetails + '_' + tWet + 'h' + "_sf" + str(scalingFactor) + '_' + timeStamp + ".xml"))
        ...
        outputFileNamepFBA = "pFBA_" +  modelName_wExpDetails + '_' + tWet + 'h' + "_sf" + str(scalingFactor) + '_' + timeStamp
        outputFileNameFBA = "FBA_" +  modelName_wExpDetails + '_' + tWet + 'h' + "_sf" + str(scalingFactor) + '_' + timeStamp
        outputFileNameFVA = "FVA_" +  modelName_wExpDetails + '_' + tWet + 'h' + "_sf" + str(scalingFactor) + '_' + timeStamp
    ...
    dfBIOM= pd.DataFrame({'BIOM': lBiomass})
    dfBIOM_scaled = dfBIOM/scalingFactor
    gL.toLog(logStrm, f"List of biomass fluxes / scaling factor: {dfBIOM_scaled['BIOM'].tolist()}\n")
